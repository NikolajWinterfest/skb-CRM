...
export const createClientTR = (data) => {
    ...
    showBtn.classList.add('personinfo__showbutton', 'contactlink-hidden')
    ...
    // Button Showmore for link's of contact's //
    const sumOfLink = clientContactsSpan.querySelectorAll('.personinfo__contactlink')
    const hiddenItems = clientContactsSpan.querySelectorAll('.personinfo__contactlink.contactlink-hidden')
    showBtn.textContent = `+${hiddenItems.length}`

    if (sumOfLink.length > 4) {
        showBtn.classList.remove('contactlink-hidden')
    }

    showBtn.addEventListener('click', () => {
        hiddenItems.forEach(item => {
            item.classList.remove('contactlink-hidden')
        })
        showBtn.remove()
    })
