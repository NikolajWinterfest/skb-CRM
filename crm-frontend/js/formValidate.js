export const validateFormOfClient = () => {
    ...
    const checkLengthReq = (input, message, name) => {
        if (input.value.length < 2) {
            message.textContent = `The ${name} must have a minimum of 2 letters`
            return false
        } else {
            message.textContent = ''
        }
    ...
    if (!checkLengthReq(clientName, errorName, 'Name')) { return false }
    if (!checkLengthReq(clientSurname, errorSurname, 'Surname')) { return false }
